...
- [ ] **지도와 동물 목록의 유기적 결합**:
  ...
  - **"이 지역 동물 보기"** 버튼 클릭 시, 동물 목록 페이지로 이동하며 주소창에 `?city=지역명` (예: `?city=경상남도`) 파라미터 전달.
- [ ] **동물 목록 페이지(`/animals/page.js`)**가 해당 `city` 쿼리를 감지해 수집된 전체 동물 중 해당 시/도 소속의 동물들만 동적으로 필터링해 보여주는지 연동 확인.
