...
export default function MatchResultsPage() {
  ...
  useEffect(() => {
    ...
    // Compute matches locally (runs immediately, fallback to FastAPI)
    const scoredAnimals = animals.map((animal) => {
      ...
      // Provide fallback recommendation reason if reasons array is empty
    ...
    // Save calculation to localStorage to share with detail page
              ...
              <div className="flex flex-col justify-between flex-grow pr-12 md:pr-16">
                <div>
                  ...
                  <h3 className="text-[18px] font-semibold leading-normal text-on-surface font-semibold">
                    {animal.breeds} <span className="text-on-surface-variant font-normal text-[15px]">• {animal.animal_age}</span>
                  </h3>
                </div>
